...
import { Users, TrendingUp, ChevronDown, ChevronUp, Settings2, Trash2 } from "lucide-react";
...
export default function Tecnologia() {
  ...
  async function deleteClinic(clinic) {
    const ok = window.confirm(
      `Excluir a clínica "${clinic.name}"?\n\nIsso remove PERMANENTEMENTE a conta e todos os dados dela (pacientes, agendamentos, etc.). Esta ação não pode ser desfeita.`
    );
    if (!ok) return;
    setDeleting(clinic.id);
    try {
      await adminApi.delete(`/admin/clinics/${clinic.id}`);
  ...
  return (
    <AdminLayout>
        <TecBreadcrumb title="Clínicas" subtitle="Gestão das clínicas cadastradas na plataforma." />

        <SecaoInfo titulo="Gerenciamento de clínicas" itens={[
          ...
          ...(owner ? [{ nome: "Excluir clínica", desc: "Remove permanentemente a clínica e todos os seus dados. Disponível apenas para o proprietário da conta." }] : []),
        ]} />
            ...
            <table className="w-full text-sm">
              <thead className="bg-[#F2F0EB] text-xs text-gray-500 uppercase tracking-wide">
                <tr>
                  ...
                  {owner && <th className="text-center px-6 py-3">Ações</th>}
                </tr>
              </thead>
              <tbody>
                {clinics.map((c) => (
                  <Fragment key={c.id}>
                    <tr className={`border-t border-[#E6E2D8] hover:bg-[#F2F0EB] transition ${expanded === c.id ? "bg-[#F2F0EB]" : ""}`}>
                      ...
                      {owner && (
                        <td className="px-6 py-4 text-center">
                          <button onClick={() => deleteClinic(c)} disabled={deleting === c.id}
                            title="Excluir clínica"
                            className="w-8 h-8 rounded-xl flex items-center justify-center mx-auto border border-[#DDD8CC] text-gray-400 hover:border-red-400 hover:text-red-500 hover:bg-red-50 transition disabled:opacity-40">
                            <Trash2 size={14} />
                          </button>
                        </td>
                      )}
                    ...
                    {expanded === c.id && (
                      <tr>
                        <td colSpan={owner ? 8 : 7} className="p-0">
